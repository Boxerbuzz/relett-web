...
Deno.serve(async (req) => {
  systemLogger('create-investment-poll: Request received', { method: req.method });
  ...
  try {
    ...
    systemLogger('create-investment-poll: Request body parsed', { pollType: body.pollType });
    ...
    if (holdingError || !tokenHolding) {
      systemLogger('create-investment-poll: User not authorized', { investmentGroupId: body.investmentGroupId });
    ...
    const pollData = {
      ...
      ends_at: body.buyoutDeadline || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(), // Default 7 days
    ...
    if (pollError) {
      systemLogger('create-investment-poll: Error creating poll', { error: pollError });
      return createResponse(handleSupabaseError(pollError), 400);
    }

    systemLogger('create-investment-poll: Poll created successfully', { pollId: poll.id });
