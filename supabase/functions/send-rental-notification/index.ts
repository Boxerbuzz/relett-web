...
Deno.serve(async (req) => {
  ...
  try {
    ...
    systemLogger("[SEND-RENTAL-NOTIFICATION]", payload);
    ...
    if (rentalError || !rental) {
      systemLogger("[SEND-RENTAL-NOTIFICATION]", rentalError);
    ...
    if (rental.user_id) {
      const { error: tenantNotifError } = await supabase
        ...
        .insert({
          ...
          type: "rental",
          title: "Rental Update",
          message: `Your rental ${maskId(
            payload.rental_id
          )} ${statusText} for ${rental.properties.title}`,
          metadata: {
      ...
      if (tenantNotifError) {
        systemLogger("[SEND-RENTAL-NOTIFICATION]", tenantNotifError);
      }
    }

    // Notify the property owner/agent
    if (
      rental.properties.user_id &&
      rental.properties.user_id !== rental.user_id
    ) {
      const { error: ownerNotifError } = await supabase
        ...
        .insert({
          ...
          message: `Your property "${rental.properties.title}" rental ${maskId(
            payload.rental_id
          )} ${statusText}`,
          metadata: {
            rental_id: payload.rental_id,
      ...
      if (ownerNotifError) {
        systemLogger("[SEND-RENTAL-NOTIFICATION]", ownerNotifError);
      }
    }
    ...
    systemLogger("[SEND-RENTAL-NOTIFICATION]", error);
    return createResponse(
      {
        success: false,
