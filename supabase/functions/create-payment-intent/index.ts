...
serve(async (req) => {
  ...
  try {
    ...
    const { amount, currency, purpose, metadata }: PaymentIntentRequest =
      await req.json();
    ...
    if (!amount || !currency || !purpose) {
      return createResponse(
        createErrorResponse(
          "Missing required fields: amount, currency, purpose"
        ),
        400
      );
    }
    ...
    return createResponse(
      createErrorResponse("Payment intent creation failed", errorMessage),
      400
    );
  }
});
