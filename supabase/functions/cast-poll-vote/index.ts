import { createTypedSupabaseClient, createAuthenticatedClient, verifyUser, handleSupabaseError, createResponse, corsHeaders, createCorsResponse } from '../shared/supabase-client.ts';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    systemLogger('cast-poll-vote: Request body parsed', { pollId: body.pollId, vote: body.vote });
    ...
    if (holdingError || !tokenHolding) {
      systemLogger('cast-poll-vote: User not authorized to vote', { investmentGroupId: poll.investment_group_id });
      return createResponse({
        success: false,
    ...
    if (poll.poll_type === 'buyout_proposal') {
      ...
      if (property && property.total_supply) {
        votingPower = (parseFloat(tokenHolding.tokens_owned) / parseFloat(property.total_supply)) * 100;
      }
    }
    ...
    if (voteCheckError) {
      systemLogger('cast-poll-vote: Error checking existing vote', { error: voteCheckError });
    }
    ...
    if (existingVote && poll.allow_vote_changes) {
      ...
      return createResponse({
        success: false,
        error: 'You have already voted on this poll and vote changes are not allowed'
      }, 400);
    }

    if (voteResult.error) {
      systemLogger('cast-poll-vote: Error saving vote', { error: voteResult.error });
    ...
    if (poll.poll_type === 'buyout_proposal' && body.vote === 'yes') {
      const { error: updateError } = await supabase
        .from('investment_polls')
        .update({
          current_buyout_votes: poll.current_buyout_votes + votingPower
        })
        .eq('id', body.pollId);

      if (updateError) {
        systemLogger('cast-poll-vote: Error updating poll vote count', { error: updateError });
      }
    }
