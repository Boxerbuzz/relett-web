...
serve(async (req) => {
  ...
  try {
    ...
    if (!listing_id || !buyer_id) {
      return createResponse(createErrorResponse('Missing listing ID or buyer ID'), 400);
    }
    ...
    // Get listing details
    const { data: listing, error: listingError } = await userResult.supabaseClient
    ...
    if (listingError || !listing) {
      return createResponse(createErrorResponse('Listing not found or no longer active'), 404);
    }

    // Check if buyer is trying to buy their own listing
    if (listing.seller_id === buyer_id) {
      return createResponse(createErrorResponse('Cannot buy your own listing'), 400);
    }

    // Get buyer's Hedera wallet
    const { data: buyerWallet, error: walletError } = await userResult.supabaseClient
      .from('hedera_wallets')
    ...
    if (walletError || !buyerWallet) {
      return createResponse(createErrorResponse('Buyer Hedera wallet not found'), 400);
    }

    // Get seller's Hedera wallet for the transfer
    const { data: sellerWallet, error: sellerWalletError } = await userResult.supabaseClient
      .from('hedera_wallets')
      .select('hedera_account_id')
    ...
    if (sellerWalletError || !sellerWallet) {
      return createResponse(createErrorResponse('Seller Hedera wallet not found'), 400);
    }

    // Ensure token association for buyer
    try {
      const { error: associationError } = await userResult.supabaseClient.functions.invoke(
    ...
    const { data: transferResult, error: transferError } = await userResult.supabaseClient.functions.invoke(
    ...
    if (transferError) {
      ...
      return createResponse(createErrorResponse('Token transfer failed', transferError.message), 500);
    }

    // Update marketplace listing status
    const { error: updateError } = await userResult.supabaseClient
      .from('marketplace_listings')
      .update({ 
        status: 'sold',
        buyer_id: buyer_id,
    ...
    const { error: sellerUpdateError } = await userResult.supabaseClient.rpc(
    ...
    for (const notification of notifications) {
      await userResult.supabaseClient.functions.invoke('process-notification', { 
        body: notification 
      });
    }
