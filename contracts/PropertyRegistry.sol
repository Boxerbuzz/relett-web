...
contract PropertyRegistry is Ownable, Pausable {
    ...
    function getVerifiedProperties() external view returns (string[] memory) {
        string[] memory verified = new string[](allPropertyIds.length);
        uint256 count = 0;
        
        for (uint256 i = 0; i < allPropertyIds.length; i++) {
            if (properties[allPropertyIds[i]].isVerified) {
                verified[count] = allPropertyIds[i];
                count++;
            }
        }
