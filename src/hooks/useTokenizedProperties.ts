...
export function useTokenizedProperties() {
  const { user } = useAuth();
  const [tokenizedProperties, setTokenizedProperties] = useState<TokenizedPropertyData[]>([]);
  ...
  const fetchTokenizedProperties = async () => {
    try {
      ...
      setTokenizedProperties(enrichedProperties);
  ...
  return {
    ...
    loading,
    error,
    refetch: fetchTokenizedProperties
  };
}
