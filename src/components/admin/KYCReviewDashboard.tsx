...
export function KYCReviewDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"documents" | "identity">(
    "documents"
  );
    ...
    try {
      ...
      const { data: docsData, error: docsError } = await supabase
        ...
        .select(
          `
          *,
          user:users!kyc_documents_user_id_fkey(first_name, last_name, phone)
        `
        )
  ...
  const handleIdentityDecision = async (
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          >
            ...
            Documents (
            {
              kycDocuments.filter((d) => d.verification_status === "pending")
                .length
            }
            )
          </Button>
              ...
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-gray-500">
                    Choose a document from the list to review and verify.
                  </p>
                </CardContent>
              </Card>
            )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {verification.verification_status === "pending" && (
                      <div className="flex gap-2">
                        <Button
                          onClick={() =>
                            handleIdentityDecision(verification.id, "verified")
                          }
                          disabled={processing}
                          size="sm"
                        ...
                        <Button
                          onClick={() =>
                            handleIdentityDecision(verification.id, "rejected")
                          }
                          disabled={processing}
                          variant="destructive"
                          size="sm"
          ...
          {identityVerifications.length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-gray-500">
                  No pending identity verifications to review at this time.
                </p>
              </CardContent>
            </Card>
          )}
