...
export function RevenueDistribution() {
  ...
  const fetchData = async () => {
    try {
      ...
      const hasAccess = roles?.some(
        (r) => r.role === "admin" || r.role === "landowner"
      );
      if (!hasAccess) {
        toast({
          title: "Access Denied",
          description:
            "You need appropriate privileges to access this feature.",
          variant: "destructive",
        });
        return;
      }
      ...
      const transformedProperties: TokenizedProperty[] = (
        propertiesData || []
      ).map((item) => ({
        id: item.id,
      ...
      const transformedDistributions = (distributionsData || []).map(
        (item) => ({
          ...item,
          tokenized_property: {
  ...
  const createDistribution = async () => {
    if (
      !selectedProperty ||
      !distributionData.totalRevenue ||
      !distributionData.sourceDescription
    ) {
      toast({
    ...
    try {
      ...
      const { error: notificationError } = await supabase
        ...
        .insert({
          ...
          message: `Revenue distribution of $${totalRevenue.toFixed(
            2
          )} has been processed for ${property.token_name}.`,
          metadata: {
            amount: totalRevenue,
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                ...
                <option value="property_appreciation">
          ...
          {selectedProperty && (
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              {(() => {
                ...
                const totalRevenue =
                  parseFloat(distributionData.totalRevenue) || 0;
                const totalSupply = property
                  ? parseFloat(property.total_supply)
                  : 0;
                const revenuePerToken =
                  totalSupply > 0 ? totalRevenue / totalSupply : 0;
                ...
                return (
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-semibold">
                        {formatCurrency(revenuePerToken)}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600">Token Holders</p>
                      <p className="font-semibold">
                        {property?.token_holdings?.length || 0}
                      </p>
                    </div>
                  </div>
                );
          ...
          <Button
            ...
            disabled={
              isCreating || !selectedProperty || !distributionData.totalRevenue
            }
            className="w-full"
          >
