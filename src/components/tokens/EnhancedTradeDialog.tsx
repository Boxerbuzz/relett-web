...
export function EnhancedTradeDialog({ isOpen, onClose, property, onTradeComplete }: EnhancedTradeDialogProps) {
  ...
  useEffect(() => {
    validateTradeAmount();
  }, [tokenAmount, tradeType, property.ownedTokens, wallet]);
  ...
  const validateTradeAmount = () => {
    ...
    if (tradeType === 'sell' && parseFloat(tokenAmount) > property.ownedTokens) {
      setValidationError(`You can only sell up to ${property.ownedTokens} tokens`);
      return;
    }
  ...
  const canTrade = wallet && !walletLoading && tokenAmount && parseFloat(tokenAmount) > 0 && !validationError;
