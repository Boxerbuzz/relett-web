...
import { CurrencyDollar, TrendUp, Shield, Clock } from '@phosphor-icons/react';
...
export function InvestNowDialog({ open, onOpenChange, tokenizedProperty }: InvestNowDialogProps) {
  const { user } = useAuth();
  const { toast } = useToast();
  const [investmentAmount, setInvestmentAmount] = useState(tokenizedProperty.minimum_investment);
  const [loading, setLoading] = useState(false);

  const tokensToReceive = Math.floor(investmentAmount / tokenizedProperty.token_price);
  const estimatedReturns = (investmentAmount * tokenizedProperty.expected_roi) / 100;
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-gray-50 rounded-lg space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-semibold">${tokenizedProperty.token_price}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">${tokenizedProperty.minimum_investment}</span>
            </div>
          </div>
          ...
          <div className="p-4 border rounded-lg space-y-3">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{tokensToReceive} {tokenizedProperty.token_symbol}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs text-yellow-800">
                ...
                <p>Real estate investments carry risk. Past performance doesn't guarantee future results. Please invest responsibly.</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              disabled={loading || investmentAmount < tokenizedProperty.minimum_investment}
