...
export function AgentChat({ agentId, context, className = "" }: AgentChatProps) {
  ...
  useEffect(() => {
    if (agent) {
      ...
      const welcomeMessage: Message = {
        ...
        content: `Hello! I'm ${agent.getConfig().name}. ${agent.getConfig().description}. How can I help you today?`,
        sender: 'agent',
        timestamp: new Date(),
        agentId
      };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const response = await agentManager.processMessage(agentId, inputMessage, context);
