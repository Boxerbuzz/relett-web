...
// Utility function to determine which fields should be visible based on property type and category
const getVisibleFields = (propertyType: string, category: string) => {
  const fieldConfig = {
    // Residential properties show most fields
    residential: {
      bedrooms: true,
      bathrooms: true,
      toilets: true,
      ...
      garages: true,
      floors: true,
      units: category === 'rent' || category === 'shortlet', // Only for rental properties
      area: true,
      year_built: true,
      is_furnished: true,
    ...
    // Commercial properties don't show bedrooms/bathrooms but show units and floors
    commercial: {
      bedrooms: false,
      bathrooms: false,
      toilets: true, // Commercial properties may have toilets
      parking: true,
      garages: false, // Commercial properties typically don't have garages
      floors: true,
      units: true, // Commercial spaces often have multiple units
      area: true,
      year_built: true,
      is_furnished: category === 'rent' || category === 'shortlet', // Only for rentals
      full_bedroom_count: false,
    },
    
    // Industrial properties focus on area, floors, and practical aspects
    industrial: {
      bedrooms: false,
      bathrooms: false,
      toilets: true,
      ...
      area: true,
      year_built: true,
      is_furnished: false, // Industrial properties are typically not furnished
      full_bedroom_count: false,
    },
    
    // Land properties only show area-related fields
    land: {
      bedrooms: false,
      bathrooms: false,
      toilets: false,
      ...
      units: false,
      area: true,
      year_built: false, // Land doesn't have a year built
      is_furnished: false,
      full_bedroom_count: false,
    },
  };
...
// Utility function to get relevant amenities based on property type
const getRelevantAmenities = (propertyType: string) => {
  const amenityConfig = {
    residential: [
      "Security System",
      "Gym",
  ...
  return amenityConfig[propertyType as keyof typeof amenityConfig] || amenityConfig.residential;
};
...
export function SpecificationStep({ form }: SpecificationStepProps) {
  ...
  // Get visible fields based on property type and category
  const visibleFields = getVisibleFields(propertyType, category);
  const relevantAmenities = getRelevantAmenities(propertyType);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-gray-600 mb-6">
          Provide detailed specifications about your {propertyType} property.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {visibleFields.full_bedroom_count && (
          <FormField
            control={form.control}
      ...
      {/* Furnished Status - Only for properties that can be furnished */}
      {visibleFields.is_furnished && (
        <FormField
          control={form.control}
