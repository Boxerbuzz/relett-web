  ...
  const fetchMarketData = async () => {
    try {
      ...
      const totalMarketValue =
        properties?.reduce((sum, prop) => sum + prop.total_value_usd, 0) || 0;
      const activeProperties = properties?.length || 0;
      const averageROI = properties?.length
        ? properties.reduce((sum, prop) => sum + prop.expected_roi, 0) /
          properties.length
        : 0;
      ...
      const topPerformingProperties: TopProperty[] =
        properties
          ?.map((prop) => {
            const investorCount = prop.token_holdings?.length || 0;
            const totalTokensOwned =
              prop.token_holdings?.reduce(
                (sum, holding) => sum + parseFloat(holding.tokens_owned),
                0
              ) || 0;
            ...
            if (prop.property?.location) {
              if (
                typeof prop.property.location === "object" &&
                prop.property.location !== null &&
                !Array.isArray(prop.property.location)
              ) {
                const locationObj = prop.property.location as Record<
                  string,
                  any
                >;
                location =
                  locationObj.city || locationObj.address || "Lagos, Nigeria";
