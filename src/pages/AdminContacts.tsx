...
export default function AdminContacts() {
  ...
  const [responseMethod, setResponseMethod] = useState<'email' | 'sms'>('email');
                            ...
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm">{contact.phone_number || 'Not provided'}</p>
                                </div>
                              </div>
                              ...
                              {contact.status !== 'responded' && (
                                <div className="space-y-4 border-t pt-4">
                                  ...
                                  <div className="flex gap-2">
                                    <Select value={responseMethod} onValueChange={(value: 'email' | 'sms') => setResponseMethod(value)}>
                                  ...
                                  >
                                    {responding ? (
                                      ...
                                      <>
                                        {responseMethod === 'email' ? <Envelope size={16} className="mr-2" /> : <Phone size={16} className="mr-2" />}
                                        Send {responseMethod === 'email' ? 'Email' : 'SMS'} Response
                                      </>
                                    )}
                                  </Button>
                                </div>
                              ...
                              {contact.status === 'responded' && (
                                <div className="bg-green-50 p-3 rounded">
                                  <p className="text-sm text-green-800">
                                    <strong>Responded via {contact.response_method} on:</strong><br />
                                    {contact.responded_at && new Date(contact.responded_at).toLocaleString()}
                                  </p>
                                  {contact.response_notes && (
                                    <div className="mt-2">
                                      <strong>Response:</strong>
                                      <p className="text-sm">{contact.response_notes}</p>
                                    </div>
                                  )}
                                </div>
                              )}
