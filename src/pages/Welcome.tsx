...
const Welcome = () => {
  ...
  const userTypeGuides = {
    user: {
      ...
      description: 'Browse properties, make investments, and track your portfolio',
      features: [
        {
    ...
    landowner: {
      ...
      description: 'List your properties, manage tokenization, and track performance',
      features: [
        {
                ...
                {featuresStatus.working.map((feature, index) => (
                  <div 
                    ...
                    className={`flex items-center gap-3 p-2 rounded cursor-pointer transition-colors
                      ${(!feature.roles || feature.roles.some(role => roles.includes(role as any))) 
                        ? 'hover:bg-green-50' : 'opacity-50'}`}
                    onClick={() => {
                      if (!feature.roles || feature.roles.some(role => roles.includes(role as any))) {
                        if (feature.path) navigate(feature.path);
                        handleFeatureDemo(feature.name);
                      }
                    }}
                  >
                    <feature.icon className={`h-4 w-4 ${activeDemo === feature.name ? 'text-green-600 animate-pulse' : 'text-green-500'}`} />
              ...
              <CardContent className="space-y-3">
                {featuresStatus.inProgress.map((feature, index) => (
                  <div key={index} className="flex items-center gap-3 p-2">
                    ...
                    <span className="text-sm font-medium">{feature.name}</span>
                    <Badge variant="outline" className="text-xs bg-yellow-50">Coming Soon</Badge>
                  </div>
                ))}
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-3">
                {featuresStatus.planned.map((feature, index) => (
                  <div key={index} className="flex items-center gap-3 p-2">
                    ...
                    <span className="text-sm font-medium">{feature.name}</span>
                    <Badge variant="outline" className="text-xs bg-blue-50">Planned</Badge>
                  </div>
                ))}
              </CardContent>
            </Card>
        ...
        <TabsContent value="quick-start" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <p className="text-gray-600">Complete these steps to get the most out of Relett</p>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100" onClick={() => navigate('/profile')}>
                  <div className="h-5 w-5 border-2 border-gray-400 rounded-full"></div>
                  <span className="flex-1">Complete your profile information</span>
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100" onClick={() => navigate('/marketplace')}>
                  <div className="h-5 w-5 border-2 border-gray-400 rounded-full"></div>
          ...
          <Card className="border border-blue-200 bg-blue-50">
            ...
            <CardContent className="text-sm text-blue-700 space-y-2">
              <p><strong>KYC Verification:</strong> Complete your identity verification in your profile settings. Admins review and approve KYC documents through the admin dashboard.</p>
              <p><strong>Investment Features:</strong> Token investment functionality is currently in development. You can browse properties and track holdings, but investment processing is not yet active.</p>
              <p><strong>Support:</strong> If you encounter any issues or have questions, please use the feedback system or contact support.</p>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
